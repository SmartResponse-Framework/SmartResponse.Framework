...
Function New-BuildInfo {
    ...
    .DESCRIPTION
        The New-BuildInfo cmdlet creates an empty BuildInfo.json file in the build directory.
    .PARAMETER Force
    ...
    if ($Force) {
        if (Test-Path $BuildPath) {
            ...
            throw [DirectoryNotFoundException] "[New-BuildInfo]: Unable to find build directory $BuildPath"
        }
    }
    ...
    if (! (Test-Path $BuildInfoPath)) {
        ...
        Write-Host "[New-BuildInfo]: BuildInfo.json file already exists, will not overwrite."
    }
}